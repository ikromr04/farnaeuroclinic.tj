<section {{ $attributes->merge([
    'class' => 'relative container group md:py-10 md:px-20 md:border md:border-brand md:rounded-[10px] md:bg-white',
]) }}>
  ...
  <h2 class="font-semibold text-[25px] leaading-[1.2] mb-2 md:mb-5">{{ __('Миссия') }}</h2>

  <div class="relative transition-all duration-300 max-h-[190px] overflow-hidden group-[.shown]:max-h-[1000px] md:max-h-none">
    <p class="font-semibold mb-2">
      {{ __('Привносить свет надежды в жизни семей, сталкивающихся с проблемами бесплодия в атмосфере заботы, понимания и научного совершенства.') }}
    </p>
    <p onclick="this.closest('section').classList.add('shown')">
    ...
    <button class="font-semibold flex mt-3 md:hidden" type="button" onclick="this.closest('section').classList.remove('shown')">
      {{ __('Свернуть') }}
    </button>
