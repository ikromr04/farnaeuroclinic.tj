...
import { TextStoreDTO, TextUpdateDTO } from '@/dto/texts-dto';
...
export const storeTextAction = createAsyncThunk<Text, {
  dto: TextStoreDTO,
  onSuccess?: () => void,
  ...
  async ({ dto, onValidationError, onSuccess, onFail }, { extra: api, rejectWithValue }) => {
    try {
      const { data } = await api.post<Text>(APIRoute.Texts.Index, dto);
