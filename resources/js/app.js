    ...
    fetch(`/program?category_id=${evt.target.dataset.categoryId}&page=${evt.target.dataset.showMore}`)
      ...
      .then((data) => {
        if (window.innerWidth < 768) {
          ...
          evt.target.previousElementSibling.previousElementSibling.insertAdjacentHTML('beforeend', data.data.map((program) => `
            <li>
              <article class="relative flex flex-col max-w-[90vw] mx-auto sm:h-[204px] sm:pt-10 sm:pr-6 sm:pb-6 sm:pl-10 sm:border sm:border-brand sm:rounded-[10px] mt-[5px]">
                ...
                ${program.article ?
              `<a class="underline" href="/services/${program.slug}">
                    Подробнее
                  </a>` : ''}

                  <a class="button-brand ml-auto" href="/programs/${program.slug}">
            ...
            <li>
              <article class="max-w-full">
                <a class="grid grid-cols-[80px_1fr] gap-5" href="/doctors/${doctor.slug}">
                  <img class="flex w-20 h-20 object-cover object-top rounded-full bg-slate-200" src="${doctor.avatar}" width="240" height="360" alt="${doctor.name}">

                  <div>
                    <h2 class="mb-3">${doctor.name}</h2>

                    <p class="font-light line-clamp-3">${doctor.position.split('\n')[0]}</p>
                  </div>
                </a>
              </article>
            </li>
            ...
            <li>
             <article class="relative flex-col md:w-full md:mt-[5px] hidden md:flex">
                ...
                <a class="relative doctor-avatar flex border border-brand rounded-[10px] w-[240px] h-[240px] overflow-hidden mb-5 md:h-[360px] md:w-full" href="/doctors/${doctor.slug}">
                  <img class="absolute w-full h-full object-cover object-top transition-all origin-top duration-300 xl:hover:scale-125 bg-slate-200" src="${doctor.avatar}" width="240" height="360" alt="${doctor.name}">
